	...
	// Ready event.
		$(function() {
			...
			// Gallery.
				$('.gallery')
					...
					.prepend('<div class="modal" tabIndex="-1"><img style="display:none" src="" /><div class="inner"></div></div>')
						.find('img')
							.on('load', function(event) {

								var $inner = $(this),
									$modal = $inner.parents('.modal');
		...
		$('.nav a').on('click',function(e){
			...
			if($('.intro-more').hasClass('intro-more-d')){
				if ($(window).width()>415) {
					...
					if($(document).scrollLeft() + 80 >= $(document).width() - $(window).width()){
						$('.nav a:eq(3)').addClass('nav-active').siblings().removeClass('nav-active');
						mini_state = 0;
					}
				}else{
					$(window).on('scroll',function(e){
			...
			$('.more-about').on('click',function(){
				...
				$('.intro-back').removeClass('intro-more-d');
				$('.more-about').addClass('intro-more-d');
			});

			$('.intro-back').on('click',function(){
				$('.intro-more').addClass('intro-more-d');
				$('.intro-back').addClass('intro-more-d');
